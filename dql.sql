...
SELECT COUNT(tb_servico.id) AS 'Total Servico', status_servico AS 'Status' 
...
SELECT titulo AS 'Servico', tb_tech.nome AS 'Tecnologia' FROM tb_tech_servico INNER JOIN tb_servico ON tb_tech_servico.id_servico = tb_servico.id INNER JOIN tb_tech ON tb_tech_servico.id_tech = tb_tech.id;
